...
import type { ServiceAuthenticator } from './service-authentication.js'

export interface PrivateApiAuthentication {
  readonly authenticator: ServiceAuthenticator
  readonly credentialFile: string
}

export async function createPrivateApiAuthentication(
  dataDirectory: string
): Promise<PrivateApiAuthentication> {
  ...
  try {
    ...
    if (!info.isFile() || info.isSymbolicLink() || (info.mode & 0o077) !== 0) {
      throw new Error('PRIVATE_API_CREDENTIAL_UNSAFE')
    }
  } catch (error) {
    if (error instanceof Error && error.message === 'PRIVATE_API_CREDENTIAL_UNSAFE') throw error
  ...
  if (!/^[A-Za-z0-9_-]{43}$/.test(credential)) throw new Error('PRIVATE_API_CREDENTIAL_INVALID')
  return {
    authenticator: new PrivateBearerAuthenticator(credential),
    credentialFile,
  }
}

class PrivateBearerAuthenticator implements ServiceAuthenticator {
  ...
  async authenticate(
    ...
    if (
      ...
      throw new UnauthorizedException({
        ...
        message: 'Private API credential was rejected',
      })
    }
    const body = request.body
    if (typeof body !== 'object' || body === null) {
      throw new BadRequestException({
        code: 'PRIVATE_API_ENVELOPE_REQUIRED',
        message: 'A versioned private service envelope is required',
      })
    }
    ...
    if (principalId !== 'svc_agent-hq') {
      throw new UnauthorizedException({
        code: 'PRIVATE_API_CALLER_INVALID',
        message: 'Private API caller was rejected',
      })
    }
