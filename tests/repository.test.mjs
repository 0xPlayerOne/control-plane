...
test('configures the Code Foundry CI baseline for the public staging-release repository', async () => {
  ...
  assert.match(config, /^runtime_ref: v0\.38\.1$/m)
  for (const runner of [
    'runner',
    'ci_runner',
    'test_runner',
...
test('generates the staging-release Code Foundry callers with parallel validation', async () => {
  ...
  assert.match(
    validation,
    /uses: 0xPlayerOne\/code-foundry\/\.github\/workflows\/validation\.yml@v0\.38\.1/
  )
  ...
  assert.match(release, /release\.yml@v0\.38\.1/)
  ...
  assert.match(releasePr, /release-pr\.yml@v0\.38\.1/)
