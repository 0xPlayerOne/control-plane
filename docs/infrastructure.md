...
- Railway staging maps to Git `staging` and the dedicated Neon `staging` branch; production maps to Git `main` and Neon `main`, but its application sources remain disconnected while cloud availability is disabled;
